...
import { ArrowLeft, ArrowRight, CheckCircle2, ClipboardCheck, ClipboardPlus, FileClock, Home, LockKeyhole, LogOut, RotateCcw, Save, ShieldCheck, Stethoscope, UserCog, Users } from 'lucide-react';
...
import { UserAccessManager } from '@/src/components/UserAccessManager';
...
type Screen = 'triage' | 'anamnese' | 'exame' | 'plano' | 'revisao' | 'retorno' | 'patients' | 'drafts' | 'access';
type ClinicalScreen = Exclude<Screen, 'patients' | 'drafts' | 'access'>;
...
const navigation: Array<{ id: Screen; label: string; icon: typeof Home; adminOnly?: boolean }> = [
  ...
  { id: 'access', label: 'Gerenciar acessos', icon: UserCog, adminOnly: true },
];
...
function ClinicalApp({ session }: { session: AppSession }) {
  ...
  const title = useMemo(() => {
    if (screen === 'patients') return 'Pacientes'; if (screen === 'drafts') return 'Atendimentos'; if (screen === 'access') return 'Gerenciar acessos'; if (screen === 'triage') return 'Triagem clínica';
  ...
  const clinical = !['patients', 'drafts', 'access'].includes(screen);
  ...
  return <div className="min-h-screen bg-[var(--page)] text-slate-900">
    ...
    <div className="mx-auto grid max-w-[1500px] grid-cols-1 lg:grid-cols-[244px_minmax(0,1fr)]"><Sidebar screen={screen} isAdmin={session.bootstrap.user.role === 'admin'} onNavigate={(id) => id === 'triage' ? startNew() : id === 'retorno' ? (reset(), goTo('retorno')) : goTo(id)} /><main className="min-w-0 px-4 py-7 sm:px-7 lg:px-10 lg:py-9"><div className="mx-auto max-w-5xl">{clinical && screen !== 'retorno' ? <Workflow current={currentStep} /> : null}<PageHeading title={title} screen={screen} lockedBy={lockedBy} />
      ...
      {screen === 'access' && session.bootstrap.user.role === 'admin' ? <UserAccessManager users={session.bootstrap.users} currentUser={session.bootstrap.user} idToken={session.idToken} demo={session.demo} onRefresh={session.refresh} /> : null}
...
function Sidebar({ screen, isAdmin, onNavigate }: { screen: Screen; isAdmin: boolean; onNavigate: (id: Screen) => void }) {
  return <aside className="hidden min-h-[calc(100vh-5rem)] border-r border-slate-200 bg-white px-4 py-7 lg:block"><nav className="space-y-1">{navigation.filter((item) => !item.adminOnly || isAdmin).map(({ id, label, icon: Icon }) => { const active = id === screen || (id === 'triage' && workflow.some((step) => step.id === screen)); return <button key={id} type="button" onClick={() => onNavigate(id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-medium ${active ? 'bg-[var(--brand-blue-soft)] text-[var(--brand-blue)]' : 'text-slate-600 hover:bg-slate-50'}`}><Icon className="size-[18px]" />{label}</button>; })}</nav><div className="mt-9 rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-center gap-2 text-[var(--brand-blue)]"><ShieldCheck className="size-4" /><p className="text-xs font-bold uppercase tracking-[0.08em]">Acesso protegido</p></div><p className="mt-2 text-xs leading-5 text-slate-600">A edição simultânea é bloqueada somente para o mesmo atendimento.</p></div><img src={`${import.meta.env.BASE_URL}brand/juliana-stuginski-barbosa.png`} alt="Juliana Stuginski Barbosa Dor Orofacial" className="mx-auto mt-9 h-auto w-40 opacity-90" /></aside>;
}
...
function PageHeading({ title, screen, lockedBy }: { title: string; screen: Screen; lockedBy: string }) { const clinical = !['patients', 'drafts', 'access'].includes(screen); return <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-[var(--brand-orange)]"><Stethoscope className="size-4" />Sistema clínico</div><h1 className="font-heading text-3xl font-semibold tracking-[-0.025em] text-[var(--brand-navy)] sm:text-4xl">{title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{screen === 'triage' ? 'As respostas dos últimos 30 dias definem o caminho inicial.' : screen === 'retorno' ? 'Novo registro longitudinal com todo o conteúdo deste retorno.' : screen === 'access' ? 'Cadastre, atualize ou desative os usuários autorizados do sistema.' : clinical ? 'Preencha apenas o que foi avaliado.' : 'A lista respeita os vínculos de acesso de cada usuário.'}</p></div>{clinical ? <div className="flex items-center gap-2 text-xs text-slate-500"><LockKeyhole className="size-4 text-[var(--brand-blue)]" />{lockedBy ? `Somente leitura: ${lockedBy}` : 'Atendimento reservado para edição'}</div> : null}</div>; }
